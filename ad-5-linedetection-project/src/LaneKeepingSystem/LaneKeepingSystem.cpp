...
void LaneKeepingSystem<PREC>::run()
{
    ...
    while (ros::ok())
    {
        ...
        if (mDebugging)
        {
            std::cout << "lpos: " << leftPosisionX << ", rpos: " << rightPositionX << ", mpos: " << estimatedPositionX << "angle" << steeringAngle << std::endl;
        ...
        if (mSaveImage)
        {
            ...
            file_name += mSaveImagePath;
            file_name += iso_time_str;
            file_name += ".bmp";

            cv::imwrite(file_name, mHoughTransformLaneDetector->getDebugFrame());

            std::cout << "file:" << file_name << "lpos: " << leftPosisionX << ", rpos: " << rightPositionX << ", mpos: " << estimatedPositionX << "angle: " << steeringAngle << std::endl;
        }
...
void LaneKeepingSystem<PREC>::drive(PREC steeringAngle)
{
    ...
    steeringAngle = steeringAngle * (30/mXycarSpeed) * 0.34f;
